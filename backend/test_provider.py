...
async def test_streaming_and_embeddings() -> None:
...
async def test_rate_limiting_retry() -> None:
    ...
    async def mock_chat_create(*args, **kwargs):  # noqa: ANN002, ANN003, ANN202  # type: ignore
        nonlocal attempts
        attempts += 1
        if attempts < 3:
            ...
            print(f"[Mock Server] Attempt {attempts}: Raising 429 RateLimitError (suggesting 1s wait)")  # noqa: E501
    ...
    print(f"Total test duration: {duration:.2f}s (Expected slightly > 2s due to 1s backoff + latency)")  # noqa: E501
...
async def test_router_rules(redis_url: str) -> None:
    ...
    res2 = await router.route(RoutingCriteria(task_type="rag_generation", require_long_context=True))  # noqa: E501
    ...
    print(f"Rule 3 (Prefer Fine-tuned=True) routed to: {res3['model_id']} (Expected: fine-tuned-rag-model)")  # noqa: E501
    ...
    print(f"Rule 4 (task_type=evaluation, prefer_fine_tuned=True) routed to: {res4['model_id']} (Expected: claude-3-5-sonnet)")  # noqa: E501
...
async def test_client_wrapper_and_redis_counters(redis_url: str) -> None:
    ...
    print(f"Redis cost counter: ${float(cost):.8f} if cost else $0.00 (Expected: matches model cost)")  # noqa: E501
